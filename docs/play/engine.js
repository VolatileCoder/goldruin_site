...
VC.Client = class {
    ...
    join(id){
        ...
        this.#peer.on('open', (myid) => {
            console.log('Peer opened (', myid, ')')
            this.#id = myid;
            console.log("connecting to ", 'vc-' + id.toLowerCase())
            this.attach(this.#peer.connect('vc-' + id.toLowerCase()));
        });
        this.#peer.on('error', (err)=>{this.onError(err)});
        this.#peer.on('disconnected', ()=>{
            if(!this.#shuttingDown){
                this.#peer.reconnect();//TODO: test
            }
        });
    }
    get id(){
    ...
    connected(){
        console.warn('connectionOpened should be overridden by derived classes')
    }
    send(data){
...
VC.Polygon = class {
    ...
    static fromData(data){
        if(data && data.length>0){
            let points = [];
            data.forEach((d)=>{points.push(VC.Point.fromData(d))});
            let val = new VC.Polygon();
            val.points = points;
            return val;
        }
        return null;
    ...
    getData(){
        let data = [];
        this.points.forEach((p)=>{data.push(p.getData())});
        return data;
    }
    ...
    isPointOnSegment(p, a, b) {
        const cross = (p.y - a.y) * (b.x - a.x) - 
                    (p.x - a.x) * (b.y - a.y);
        if (Math.abs(cross) > 1e-9) return false;

        const dot = (p.x - a.x) * (b.x - a.x) + 
                    (p.y - a.y) * (b.y - a.y);
    ...
    isPointInsidePolygon(point, points) {
        ...
        for (let i = 0, j = points.length - 1; i < points.length; j = i++) {
            if (this.isPointOnSegment(point, points[j], points[i])) {
                return true; // edge counts as inside
            }
    ...
    intersect(other) {
        ...
        const subj = this.points.map(p => ({
            X: Math.round(p.x * scale),
            Y: Math.round(p.y * scale)
        }));

        const clip = other.points.map(p => ({
            X: Math.round(p.x * scale),
            Y: Math.round(p.y * scale)
        }));
        ...
        // If multiple polygons result, return largest (common in concave overlap)
        const largest = solution.sort((a,b) => b.length - a.length)[0];

        const resultPoints = largest.map(p =>
            new VC.Point(p.X / scale, p.Y / scale)
        );
...
VC.Server = class {
    ...
    #generateHostKey() {
        let key = "";
        let tokens = ['Z','X','W','Y','B','E','G','H','J','K','L','M','N','Q','T','V'];
        for(var i = 0; i< 4; i++){
            key = key + tokens[VC.Math.random(0,tokens.length-1)];
        }
        return key;
    }
    ...
    startHost(){
        ...
        console.log("starting host: ", 'vc-'+this.#id.toLowerCase())
        ...
        this.#host.on('error', (err)=>{this.onError(err)});
        this.#host.on('disconnected', ()=>{
            if(!this.#shuttingDown){
                this.#host.reconnect();//TODO: test
            }
        });
    }
    ...
    addConnection(conn){
        conn.on('error', (err)=>{this.onError(err)});
        conn.on('data', (data)=>{this.received(data)});
        conn.on('open', ()=>{console.log("connected to client")});
        this.connections.push(conn);
    }

    stop(){
        console.log("stopping host: ", this.#id)
        this.#shuttingDown = true;
...
VC.Triangle = class {
    ...
    static fromData(data){
        return new VC.Triangle(VC.Point.fromData(data[0]), VC.Point.fromData(data[1]), VC.Point.fromData(data[2]));
    }
    getData(){
        return [
            this.p1.getData(),
